...
func runMCP(ctx context.Context, reg *registry.Registry, args []string, cfg Config) int {
	return mcp.RunContextWithOptions(ctx, reg, args, mcp.Options{Addr: cfg.Addr, BearerTokens: cfg.BearerTokens})
}

// mcpHTTPHandler 暴露流式 HTTP 工具端点，供 serve 模式挂载 /mcp。
func mcpHTTPHandler(reg *registry.Registry, cfg Config) (http.Handler, bool) {
	h, err := mcp.HTTPHandler(reg, mcp.Options{BearerTokens: cfg.BearerTokens})
	if err != nil || h == nil {
		return nil, false
	}
	return h, true
